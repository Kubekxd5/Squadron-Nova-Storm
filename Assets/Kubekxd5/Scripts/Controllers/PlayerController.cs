...
public class PlayerController : MonoBehaviour
{
    ...
    private void HandleInput()
    {
        ...
        if (slotsManager != null)
        {
            if (Input.GetButton("Fire1")) slotsManager.FirePrimaryWeapons();

            if (Input.GetButton("Fire2")) slotsManager.FireSecondaryWeapons();

            if (Input.GetButton("Jump")) slotsManager.FireHangarBay();

            if (Input.GetButton("Fire3")) slotsManager.FireSpecialWeapon();
        }
    }
}